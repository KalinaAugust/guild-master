...
import { createDirectMessage, InvalidDirectMessageError } from './createDirectMessage';
...
describe('createDirectMessage', () => {
  it('throws when not authenticated', async () => {
    ...
    await expect(createDirectMessage('p1', 'hi')).rejects.toThrow('Not authenticated');
  });

  it('rejects empty body', async () => {
    useClient({ id: 'u1' }, vi.fn());
    await expect(createDirectMessage('p1', '   ')).rejects.toBeInstanceOf(InvalidDirectMessageError);
  });

  it('rejects body over 2000 chars', async () => {
    useClient({ id: 'u1' }, vi.fn());
    await expect(createDirectMessage('p1', 'a'.repeat(2001))).rejects.toBeInstanceOf(InvalidDirectMessageError);
  });

  it('inserts trimmed body and returns mapped message', async () => {
    const from = vi.fn().mockReturnValueOnce(query({ data: {
      id: 'dm1', sender_id: 'u1', recipient_id: 'p1', body: 'hi', attachment_url: null,
      created_at: 't', updated_at: 't',
      sender: { public_id: 'pub1', full_name: 'Me', avatar_url: null, alias: null, display_as_alias: false, icon: null },
    } }));
    useClient({ id: 'u1' }, from);

    const result = await createDirectMessage('p1', '  hi  ');
