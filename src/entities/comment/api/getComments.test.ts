...
describe('getComments', () => {
  it('maps rows to EventComment shape', async () => {
    const from = vi.fn().mockReturnValueOnce(query({ data: [
      { id: 'c1', event_id: 'e1', user_id: 'u2', body: 'hi', created_at: '2026-06-05T10:00:00Z', updated_at: '2026-06-05T10:00:00Z', profiles: { public_id: 'pub2', full_name: 'Bob', avatar_url: 'a.png', alias: null, display_as_alias: false, icon: null } },
    ] }));
    useClient(from);

    const result = await getComments('e1');
    expect(result).toEqual([
      { id: 'c1', eventId: 'e1', userId: 'u2', body: 'hi', createdAt: '2026-06-05T10:00:00Z', updatedAt: '2026-06-05T10:00:00Z', profile: { publicId: 'pub2', fullName: 'Bob', avatarUrl: 'a.png', alias: null, displayAsAlias: false, icon: null } },
    ]);
  });
