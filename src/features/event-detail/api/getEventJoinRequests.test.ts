...
describe('getEventJoinRequests', () => {
  it('maps pending requests with profile data', async () => {
    const from = vi.fn().mockReturnValue(query({ data: [
      { id: 'r1', user_id: 'u2', created_at: 't1', profiles: { full_name: 'Bob', avatar_url: 'a.png' } },
    ...
    await expect(getEventJoinRequests('e1')).resolves.toEqual([
      { id: 'r1', userId: 'u2', userName: 'Bob', avatarUrl: 'a.png', createdAt: 't1' },
      { id: 'r2', userId: 'u3', userName: null, avatarUrl: null, createdAt: 't2' },
    ]);
  });
