...
`/profile/[publicId]` is now the single profile URL (the `/users/*` prefix from the original design was dropped):

- The header menu "Profile" item links straight to `/profile/<own public_id>`, so the shareable URL is immediately visible in the address bar.
...
- Bare `/profile` redirects to `/profile/<own public_id>` (kept for old bookmarks; still auth-protected).
- Profile URLs use a short 8-char base62 `profiles.public_id` (unique, generated by a DB default via `generate_public_id()`, backfilled) instead of the uuid: `/profile/a1B2c3D4`. The route segment is `[publicId]`; ownership is checked by comparing the resolved profile's uuid with the session user.
- `src/proxy.ts` exempts `/profile/<segment>` (regex `/^\/profile\/[^/]+/`) from the login redirect; bare `/profile` remains protected.
